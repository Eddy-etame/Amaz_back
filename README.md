...
**Bootstrap vs restart:** `npm run db:bootstrap` (and `db:seed`) are **manual, one-time** setup steps when you first clone the project or after you intentionally reset data. **Restarting Docker or Node does not wipe the database** and does not automatically re-seed. Demo seeds are for **development only** — they replace seed rows on `npm run db:mysql:seed` when you run the seed script, not on every DB restart.
...
- **Product-service** also reads **MySQL** (`MYSQL_*`) to enforce **vendor approval** on catalog mutations.
- **Admin-service:** `INTERNAL_SHARED_SECRET`, `USER_SERVICE_URL`, `MYSQL_*` (or `DATABASE_URL`), optional `ADMIN_SESSION_SECRET` — see `docs/ADMIN_RUNBOOK.md`.
...
**Docker full stack:** `npm run db:bootstrap` uses `MYSQL_HOST` from `.env` (often `localhost:3306`). That must be the **same** MySQL instance the containers use (port published from `amaz-mysql`). If you have another MySQL on `3306`, bootstrap may update the wrong database and the app will still error (e.g. MySQL `ER_BAD_FIELD_ERROR`). In that case run migrations **inside** Compose:
